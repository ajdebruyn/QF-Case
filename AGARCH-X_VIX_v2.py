...
def negative_log_likelihood(params, returns):
    ...
    for t in range(1, T):
        ...
        sigma2[t] = omega + (alpha1 + alpha2 * indicator) * eps[t-1]**2 + beta * sigma2[t-1] + gamma * vix[t]  # GARCH recursion
...
# === Parameter bounds ===
bounds = [(0, 10), (0, 10), (0, 1), (0, 1), (0.001, 1), (0, 1)]
...
def filter_garch(params, returns):
    ...
    for t in range(1, T):
        ...
        sigma2[t] = omega + (alpha1 + alpha2 * indicator) * eps[t-1]**2 + beta * sigma2[t-1] + gamma * vix[t-1]
