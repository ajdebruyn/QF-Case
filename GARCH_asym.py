...
def negative_log_likelihood(params, returns):
    mu, omega, alpha1, alpha2, beta = params
    T = len(returns)
    sigma2 = np.zeros(T)
    eps = returns - mu
    ...
    for t in range(1, T):
        ...
        sigma2[t] = omega + (alpha1 * indicator + alpha2 * (1 - indicator)) * eps[t-1]**2 + beta * sigma2[t-1]
