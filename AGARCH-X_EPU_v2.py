...
epu = data['daily_policy_index']  # Economic Policy Uncertaincy index values
...
# === Define negative log-likelihood function for AGARCH-X ===
def negative_log_likelihood(params, returns):
    mu, omega, alpha1, alpha2, beta, gamma = params
    T = len(returns)
    sigma2 = np.zeros(T)
    eps = returns - mu
    sigma2[0] = np.var(returns)  # starting value

    for t in range(1, T):
        indicator = 1 if eps[t-1] < 0 else 0  # 1 if negative
        sigma2[t] = omega + (alpha1 + alpha2 * indicator) * eps[t-1]**2 + beta * sigma2[t-1] + gamma * epu[t]  # GARCH recursion
...
def filter_garch(params, returns):
    ...
    for t in range(1, T):
        ...
        sigma2[t] = omega + (alpha1 + alpha2 * indicator) * eps[t-1]**2 + beta * sigma2[t-1] + gamma * epu[t-1]
