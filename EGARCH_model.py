...
# === Define negative log-likelihood function for EGARCH ===
def negative_log_likelihood(params, returns):
    mu, omega, alpha, beta, gamma = params
    T = len(returns)
    logsigma2 = np.zeros(T)
    eps = returns - mu  # Residuals
    logsigma2[0] = np.log(np.var(returns))  # Initialize with log of sample variance
    
    for t in range(1, T):
        logsigma2[t] = omega + alpha * abs(eps[t-1] / np.sqrt(np.exp(logsigma2[t-1]))) + beta * logsigma2[t-1] + gamma * eps[t-1] / np.sqrt(np.exp(logsigma2[t-1]))  # EGARCH recursion

    loglikelihood = 0.5 * (np.log(2*np.pi) + logsigma2 + eps**2 / np.exp(logsigma2))
...
# === Parameter bounds ===
bounds = [(-10, 10), (-10, 10), (-1, 1), (-1, 1), (-1, 1)]
...
def filter_garch(params, returns):
    ...
    for t in range(1, T):
        logsigma2[t] = omega + alpha * abs(eps[t-1] / np.sqrt(np.exp(logsigma2[t-1]))) + beta * logsigma2[t-1] + gamma * eps[t-1] / np.sqrt(np.exp(logsigma2[t-1]))
...
plt.plot(dates, np.sqrt(sigma2_filtered), 'r', label=r'Estimated $\sigma_{t|t-1}$ from EGARCH')
plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
plt.xticks(pd.date_range('2000-01-01', '2024-01-01', freq='YS').to_pydatetime(), rotation=0)
plt.xlim([pd.Timestamp('2000-01-01'), pd.Timestamp('2025-01-01')])
plt.ylim([-15, 15])
...
plt.plot(dates, sigma2_filtered, 'r:', linewidth=2, label=r'Estimated $\sigma^2_{t|t-1}$ from EGARCH')
plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
plt.xticks(pd.date_range('2000-01-01', '2024-01-01', freq='YS').to_pydatetime(), rotation=0)
plt.xlim([pd.Timestamp('2000-01-01'), pd.Timestamp('2025-01-01')])
plt.ylim([0, 100])
